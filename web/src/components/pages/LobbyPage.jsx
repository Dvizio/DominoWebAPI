...
function LobbyPage() {
  ...
  const [gameId] = useState(() => {
    ...
    const gId =
      state?.lobby?.gameId ||
      state?.gameId ||
      sessionStorage.getItem("domino_current_gameId") ||
      "";
    ...
    const connectToHub = async () => {
      try {
        ...
        hubConnection.on("PlayerDisconnected", (msg) => {
          console.log(`player with id ${msg} passed out`);
        });
        ...
        hubConnection.onreconnected(async () => {
          ...
          try {
            ...
            console.error(
              "Error re-joining game group after reconnect:",
              rejoinErr,
            );
          }
        });
  ...
  const deckName =
    settings.deckSize === 6
      ...
      : settings.deckSize === 9
        ...
        : "Double 12 (91 tiles)";
  const ruleName =
    settings.startingRule === 0 ? "Highest Double" : "Random Selection";
      ...
      <main className="lobby-main">
        ...
        <div className="lobby-card">
          <div className="card-header">
            <h2 className="card-title">Seated Players</h2>
          ...
          <ul className="players-list">
            {lobby.players.map((player) => {
              ...
              return (
                <li
                  key={player.playerId}
                  className={`player-item-card ${isMe ? "is-you" : ""}`}
                >
          ...
          {lobby.players.length < 2 && (
            <div className="waiting-players-box">
              <span>
                Waiting for players to join (Need at least 2 to start)
              </span>
            </div>
          )}
        </div>
        ...
        <div className="lobby-card">
          <div className="card-header">
            <h2 className="card-title">Table Rules & Settings</h2>
            {isHost && (
          ...
          {isHost ? (
            ...
            >
              <div className="settings-grid">
                ...
                <div className="settings-field">
                  <label className="settings-label" htmlFor="deckSize">
                    Deck Size
                  </label>
                  <select
                    id="deckSize"
                ...
                <div className="settings-field">
                  <label className="settings-label" htmlFor="handSize">
                    Hand Size
                  </label>
                  <input
                    id="handSize"
                ...
                <div className="settings-field">
                  <label className="settings-label" htmlFor="targetScore">
                    Target Score (pts)
                  </label>
                  <input
                    id="targetScore"
                ...
                >
                  <label className="settings-label" htmlFor="startingRule">
                    First Turn Rule
                  </label>
                  <select
                    id="startingRule"
